module.exports = {
     ...
     */
    returnCandidates: (db, candidate, showDeleted = false, showComplete = false) => {
        return new Promise((resolve) => {
            let append = ``;
            if (showDeleted && showComplete) {
            ...
            } else if (showDeleted && !showComplete) {
                db.query(`SELECT * FROM candidates ${candidate.email.length ? `WHERE email='${candidate.email.trim().toLowerCase()}' AND ` : `WHERE `} completed = 'false'; `).then((candidateList) => {
                    resolve(candidateList.rows);
                }).catch(() => {
                    resolve([]);
                });
            } else if (!showDeleted && showComplete) {
                db.query(`SELECT * FROM candidates ${candidate.email.length ? `WHERE email='${candidate.email.trim().toLowerCase()}' AND ` : `WHERE `} deleted = 'false'; `).then((candidateList) => {
                    resolve(candidateList.rows);
                }).catch(() => {
                    resolve([]);
                });
            } else {
                db.query(`SELECT * FROM candidates ${candidate.email.length ? `WHERE email='${candidate.email.trim().toLowerCase()}' AND ` : `WHERE `} deleted = 'false' AND completed = 'false'; `).then((candidateList) => {
                    resolve(candidateList.rows);
                }).catch(() => {
                    resolve([]);
                });
    ...
    editCandidateCompleteness: (db, email, value) => {
        return new Promise((resolve) => {
            db.query(`UPDATE candidates SET completed = ${value} WHERE email = '${email.trim().toLowerCase()}';`)
                .then(() => {
                    resolve(true);
                }).catch(() => {
                    resolve(false);
    ...
    updateCandidateEmail: (db, currentEmail, newEmail) => {
        return new Promise((resolve) => {
            db.query(`UPDATE candidates SET email='${newEmail.trim().toLowerCase()}' WHERE email='${currentEmail.trim().toLowerCase()}';`)
                .then(() => {
                    resolve(true);
                }).catch((response) => {
                    resolve(false);
     ...
     */
    deleteCandidate: (db, candidateInformation, preventImportAgain = false) => {
        return new Promise((resolve) => {
            ...
            new Promise((resolve, reject) => {
                ...
                db.query(`
                    DELETE FROM recruiting_checklist_people        WHERE email='${email}';
                    DELETE FROM it_checklist_people                WHERE email='${email}';
                    DELETE FROM office_management_checklist_people WHERE email='${email}';
                    DELETE FROM operations_checklist_people        WHERE email='${email}';
                    DELETE FROM training_checklist_people          WHERE email='${email}';
                    DELETE FROM hr_checklist_people                WHERE email='${email}';
                    `).then(() => {
                    resolve(true);
                }).catch(() => {
                    reject(false);
                }).then(() => {
                    if (preventImportAgain) {
                        db.query(`UPDATE candidates SET deleted = 'true' WHERE email='${email}';`)
                            .then(() => {
                                resolve(true);
                            }).catch(() => {
                                resolve(false);
    ...
    allowImportAgain: (db, candidateInformation) => {
        return new Promise((resolve, reject) => {
            let email = candidateInformation.email;
            db.query(`UPDATE candidates SET deleted = 'false' WHERE email='${email}';`)
                .then(() => {
                    resolve({
