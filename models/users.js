module.exports = {
    ...
    createUser: (db, userInformation) => {
        return new Promise((resolve) => {
            db.query(`INSERT INTO credentials (email, password, first_name, last_name, hash_verify, verified, department) VALUES 
            ('${userInformation.email.trim().toLowerCase()}', 
            '${userInformation.hashedPassword}', 
            '${userInformation.firstName}', 
            '${userInformation.lastName}',
            '${userInformation.hashVerify}',
             ${false}, 
    ...
    verifyUser: (db, userEmail) => {
        return new Promise((resolve) => {
            db.query(`UPDATE credentials SET verified = true WHERE email = '${userEmail}';`)
                .then(() => {
                    resolve(true);
                }).catch(() => {
                    resolve(false);
